...
IF OBJECT_ID('DAVID_Y_LOS_COCODRIDLOS.MIGRACION_RENDICION') IS NOT NULL
DROP PROCEDURE DAVID_Y_LOS_COCODRIDLOS.MIGRACION_RENDICION

IF OBJECT_ID('DAVID_Y_LOS_COCODRIDLOS.MIGRACION_ITEM_RENDICION') IS NOT NULL
DROP PROCEDURE DAVID_Y_LOS_COCODRIDLOS.MIGRACION_ITEM_RENDICION

IF OBJECT_ID('DAVID_Y_LOS_COCODRIDLOS.MIGRACION_FACTURA') IS NOT NULL
DROP PROCEDURE DAVID_Y_LOS_COCODRIDLOS.MIGRACION_FACTURA

IF OBJECT_ID('DAVID_Y_LOS_COCODRIDLOS.MIGRACION_ITEM_FACTURA') IS NOT NULL
...
CREATE TABLE DAVID_Y_LOS_COCODRILOS.ITEM_RENDICION (
	...
	primary key(ITEM_ID),
--	foreign key(RENDICION_ID) references DAVID_Y_LOS_COCODRILOS.RENDICION
);
	...
	INSERT INTO DAVID_Y_LOS_COCODRILOS.AUTOMOVIL (
		...
		AUTOMOVIL_HABILITADO
				...
				(	SELECT c.USUARIO_ID
					...
					WHERE c.AUTOMOVIL = m.Auto_Patente
				), 
				1
...
BEGIN
	...
	INSERT INTO DAVID_Y_LOS_COCODRILOS.VIAJE (
		...
		VIAJE_CLIENTE_ID
				...
				(	SELECT c.USUARIO_ID
					FROM DAVID_Y_LOS_COCODRILOS.CHOFER c
					WHERE m.Auto_Patente = c.AUTOMOVIL
				),
				(	SELECT t.TURNO_ID
